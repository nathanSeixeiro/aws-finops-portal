...
class DashboardSnapshotService:
    ...
    def _build_snapshot(self) -> dict:
        ...
        # Today's cost = yesterday's ingested data (may be in previous month)
        if yesterday.startswith(current_month):
            for r in daily_records:
                if r.period == yesterday:
                    today_usd += r.amount_usd
                    today_brl += r.amount_brl
        else:
            yesterday_recs = self._cost_repo.query_by_gran_period("DAILY", yesterday)
            today_usd = sum(r.amount_usd for r in yesterday_recs)
            today_brl = sum(r.amount_brl for r in yesterday_recs)
        ...
        # Use MONTHLY records where available, supplement with daily aggregation
        ...
        # Fill gaps from daily data (aggregate daily records by YYYY-MM)
        for r in trend_records:
            month_key = r.period[:7]  # "2026-03-25" -> "2026-03"
            if month_key not in monthly_by_period:
                monthly_by_period[month_key] = {"usd": Decimal("0"), "brl": Decimal("0")}
            # Only add if this month wasn't already covered by MONTHLY records
        # Re-aggregate daily into months for any month without MONTHLY data
        daily_by_month: dict[str, dict] = {}
        for r in trend_records:
            month_key = r.period[:7]
            if month_key not in daily_by_month:
                daily_by_month[month_key] = {"usd": Decimal("0"), "brl": Decimal("0")}
            daily_by_month[month_key]["usd"] += r.amount_usd
            daily_by_month[month_key]["brl"] += r.amount_brl
        for mk, totals in daily_by_month.items():
            if mk not in monthly_by_period or (monthly_by_period[mk]["usd"] == 0 and totals["usd"] > 0):
                monthly_by_period[mk] = totals
