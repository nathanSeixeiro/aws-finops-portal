...
class CostRecordRepository:
    ...
    def query_by_gran_period(self, granularity: str, period: str) -> list[CostRecord]:
        ...
        return [CostRecord.from_dynamodb_item(item) for item in items]

    def query_by_gran_period_range(
        self, granularity: str, period_start: str, period_end: str
    ) -> list[CostRecord]:
        """Query GSI gsi-gran-period with a sort-key range on period.

        Returns all records where granularity matches and period is between
        period_start and period_end (inclusive). Handles pagination.
        """
        items = []
        kwargs = {
            "IndexName": "gsi-gran-period",
            "KeyConditionExpression": (
                Key("granularity").eq(granularity)
