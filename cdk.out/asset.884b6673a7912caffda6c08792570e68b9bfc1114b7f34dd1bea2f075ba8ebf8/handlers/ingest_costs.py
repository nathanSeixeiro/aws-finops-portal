...
def handler(event, context):
    ...
    # Support monthly backfill: {"detail": {"granularity": "MONTHLY", "backfill_months": 12}}
    elif detail.get("backfill_months") and granularity == "MONTHLY":
        import datetime
        total = 0
        today = datetime.date.today()
        year, month = today.year, today.month
        for _ in range(int(detail["backfill_months"])):
            month -= 1
            if month == 0:
                month = 12
                year -= 1
            ym = f"{year:04d}-{month:02d}"
            result = ingestion_svc.ingest_month(ym)
            total += result.get("records", 0)
        ingestion_result = {"status": "ok", "records": total}
